...
var ConfirmOrder = serverUtil.requireWebComponentFrom("confirmorder");
...
var confirmOrder = function(req, res, next) {
    ...
    bluebird.props({
        orderByParam: util.apiRequest(config.api.orderByParam.url, {
        ...
        if (resp.orderByParam.code === "success") {
            ...
            var markup = serverUtil.renderWebComponent(ConfirmOrder({
...
var submitOrder = function(req, res, next) {
    ...
    util.apiRequest(config.api.submitOrder.url, {
        memberId: user.memberId,
        itemCodes: itemIds,
        qtys: buyeds,
        couponNo: couponNo,
