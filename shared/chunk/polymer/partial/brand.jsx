    ...
    renderRecommendBrands(){
        ...
        if(recommendBrands){
            return recommendBrands.map((brand,i)=>{
                return <a href={"/search?brandName="+brand.englishName} key={i}><div><img src={brand.imageUrl}/></div></a>
            })
        }
        return null
    }
        ...
        if(categories){
            return categories.map((category,i)=>{
                let brands = category.brands.map((brand,k)=>{
                    return <a href={"/search?brandName="+brand.englishName} key={k}><div><img src={brand.imageUrl}/></div></a>
                })
                if(brands){
                    return (
